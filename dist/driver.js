import { Component, disposeDetector, Dynamic, State } from "./index.js";
...
class RenderEngine {
    node;
    app;
    dynamicChangeDetectorStack = [];
    TrackDOMLife = new Set();
        ...
        const callback = () => {
            ...
            for (let context of this.TrackDOMLife) {
                if (!context.element.isConnected) {
                    context.callback();
                    this.TrackDOMLife.delete(context);
                }
                index++;
            }
    ...
    HandleListView(listView, root) {
        ...
        for (let value of listView.from.get()) {
            ...
            root.appendChild(this.BuildDOMTree(listView.builder(_state)));
            iterator++;
        }
        const DetectChanges = (event) => {
            let OldArray = event.event == "update" ? event.value : event.value.get(listView.from);
            let NewArray = listView.from.get();
            ...
            let OldArrayMap = new Map();
            let NewArrayMap = new Map();
            ...
            for (let value of NewArrayMap) {
                ...
                if (OldArrayMap.has(value[0]) == false) {
                    if (root.children[value[1] + padding] != undefined) {
                        ...
                        root.children[value[1]].before(this.BuildDOMTree(listView.builder(IndexValueMap[value[1] + padding])));
                    }
                    else {
                        const _state = new State({ value: value[0], index: value[1] + padding });
                        _state.listen(() => {
                        ...
                        IndexValueMap.push(_state);
                        root.appendChild(this.BuildDOMTree(listView.builder(IndexValueMap[value[1] + padding])));
                    }
                    padding++;
                }
            }
    ...
    BuildDOMTree(component, initialNodeStack = [], root = 0) {
        const HandleComponentChildren = (el, ch) => {
            for (let child of ch) {
                el.append(this.BuildDOMTree(child));
            }
        };
        const HandleComponentProperties = (el, properties) => {
            for (let key of Object.keys(properties)) {
                if (properties[key] == "__ignore__" || properties[key] == undefined)
                    continue;
                if (key == "children" || key == "child" || key == "__driver__" || key == "reference" || key == "ondestroy" || key == "from" || key == "builder")
                    continue;
                switch (key) {
                    case "style":
                        if (properties.style instanceof Dynamic) {
                            ...
                            function ListenStyleChange() {
                                let newStyles = properties.style.callback();
                                if (prevCondition != properties.style.condition.get()) {
                                    let prevMap = new Map(styleKeys.map(value => [value, false]));
                                    ...
                                    prevCondition = properties.style.condition.get();
                                }
                                else if (properties.style.condition.get() == null) {
                                    let prevMap = new Map(styleKeys.map(value => [value, false]));
                                    styleKeys = Object.keys(newStyles);
                                    for (let style of styleKeys) {
                                        prevMap.set(style, true);
                            ...
                            for (let style of Object.keys(properties.style)) {
                                if (properties.style[style] == "" || properties.style[style] == undefined)
                                    continue;
        ...
        if (typeof (component) == "function") {
            ...
            let NodeStack = initialNodeStack;
            const DetectChanges = () => {
                const callback = component.callback();
                if (NodeStack.length == 0) {
                    const temp = this.BuildDOMTree(callback, NodeStack, root + 1);
                    NodeStack.push(temp);
                    return;
                }
            ...
            switch (component.name) {
                case ComponentIndex.Portal:
                    element = document.querySelector(component.properties.__driver__.selector),
                        element.replaceWith(this.BuildDOMTree(component.properties.__driver__.component));
                ...
                case ComponentIndex.ListView:
                    ...
                    this.HandleListView(component.properties.__driver__, element);
                    return element;
                ...
                case ComponentIndex.HTML:
                    const HTMLData = component.properties.__driver__;
                    element = document.createElement('div');
    ...
    Render() {
        ...
        this.node.replaceWith(this.BuildDOMTree(this.app()));
    }
}
export function RenderWebPlatform(args) {
    const Engine = new RenderEngine(document.querySelector(args.selector), args.app);
    Engine.Render();
}
